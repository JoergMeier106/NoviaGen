...
class SystemOperationsState {
  SystemOperationsState({
    required this.api,
  ...
  Future<void> loadErrors({bool silent = false}) async {
    ...
    try {
      errors = await client.fetchErrors();
    } catch (error) {
      if (!silent) {
        _setRequestError(
          error,
          'Couldn\'t load errors right now. Please try again.',
        );
      }
    } finally {
      if (!silent) {
  ...
  Future<void> clearErrors() async {
    final client = api();
    if (client == null || clearingErrors) {
      return;
    }
    clearingErrors = true;
    onChanged();
    try {
      final deleted = await client.clearErrors();
      errors = <AppErrorRecord>[];
      setMessage(
        deleted == 1 ? '1 error cleared.' : '$deleted errors cleared.',
      );
    } catch (error) {
      _setRequestError(
        error,
        'Couldn\'t clear the error log right now. Please try again.',
      );
    } finally {
