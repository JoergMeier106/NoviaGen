...
class JobPersistenceMixin:
    ...
    def get_job_for_parent_step(
        ...
        with self._write_lock(), self._connect() as conn:
            row = conn.execute(
                ...
                SELECT *
                FROM jobs
                WHERE parent_job_id = ? AND chain_step_id = ?
                ORDER BY created_at DESC
                LIMIT 1
                """,
                (parent_job_id, chain_step_id),
            ).fetchone()
        return self._row_to_dict(row) if row else None
    ...
    def list_jobs_by_parent(self, parent_job_id: str) -> list[dict[str, Any]]:
        with self._write_lock(), self._connect() as conn:
            rows = conn.execute(
                """
                SELECT *
