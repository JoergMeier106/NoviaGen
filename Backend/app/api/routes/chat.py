...
from ..responses.chat_sessions import (
...
from ..responses.chat_stream import (
...
from .context import (
    ...
    db as _db,
    errors as _errors,
    mcp_tool_provider as _mcp_tool_provider,
...
from ...chat.mcp_tools import ChatMcpConfigurationError, ChatMcpToolError
...
def list_chat_tools() -> Response:
    try:
        tools = [tool.payload() for tool in _mcp_tool_provider().list_tools()]
    except ChatMcpConfigurationError as exc:
        return jsonify({"error": str(exc)}), 503
    except ChatMcpToolError as exc:
...
def stream_chat_session_message(session_id: str) -> Response:
    return _chat_session_stream_response(
        ...
        mcp_tool_provider=_mcp_tool_provider(),
