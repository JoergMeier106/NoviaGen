...
from ...chat.tool_loop import run_chat_tool_loop, tool_trace
...
class ChatMessageJobExecutor:
    ...
    def run(
        ...
        *,
        ...
        chat_messages = [*self._build_chat_history_messages(session=session)]
        chat_messages.append(user_message_payload)
        assistant_message_id = str(uuid.uuid4())
        enabled_tool_ids = list(job.payload.get("enabled_tool_ids") or [])
        if enabled_tool_ids:
            stream_result = self._tool_response(
                job=job,
                model_name=model_name,
        ...
        assistant_message = self._create_assistant_message(
            ...
            tool_traces=stream_result.get("tool_traces"),
    ...
    def _tool_response(
        ...
        *,
        ...
        if self._mcp_tool_provider is None:
            raise RuntimeError("Chat MCP tools are not configured.")
        enabled_tool_ids = [
            str(item).strip()
            for item in job.payload.get("enabled_tool_ids", [])
            if str(item).strip()
        ]
        toolset = self._mcp_tool_provider.toolset_for_ids(enabled_tool_ids)
        running_tool_traces: list[dict[str, str]] = []
        self._sync_stream_state(
            job_id=job.job_id,
            progress=0.12,
            status_text="Preparing MCP tools",
            session_id=str(job.payload.get("session_id") or ""),
            assistant_message_id="",
            thinking_text="",
            content_text="Preparing MCP tools...",
        )

        def status_callback(event: dict[str, Any]) -> None:
            raise_if_cancel_requested(job.job_id)
            label = str(event.get("tool_id") or event.get("tool_name") or "tool")
            status = str(event.get("status") or "running")
            content = str(event.get("content") or event.get("error") or "").strip()
            if status in {"completed", "failed"} and content:
                running_tool_traces.append(
                    tool_trace(
                        tool_id=str(event.get("tool_id") or ""),
                        tool_name=str(event.get("tool_name") or label),
                        content=content,
                    )
                )
            self._sync_stream_state(
                job_id=job.job_id,
                progress=0.2,
                status_text=f"Tool {status}: {label}",
                session_id=str(job.payload.get("session_id") or ""),
                assistant_message_id="",
                thinking_text="",
                content_text=f"Tool {status}: {label}",
                tool_traces=running_tool_traces,
            )
        ...
        result = run_chat_tool_loop(
            ...
            model=model_name,
            messages=chat_messages,
            tool_provider=self._mcp_tool_provider,
            toolset=toolset,
            think=job.payload.get("think"),
            context_window=context_window,
            max_steps=self._mcp_tool_provider.max_loop_steps(),
    ...
    def _sync_stream_state(
        ...
        *,
        ...
        thinking_text: str,
        content_text: str,
        tool_traces: list[dict[str, str]] | None = None,
    ) -> None:
        self._db.update_job(
            job_id,
            status="running",
